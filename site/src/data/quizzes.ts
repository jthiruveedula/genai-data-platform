 ...
 * ModuleQuiz.astro can look one up with a single import. Sits alongside
 * ModuleRecap, not in place of it — the recap is the closing "what should
 * stick" beat, the quiz is a self-check a learner can retake.
 */
export const QUIZZES: Record<string, QuizQuestion[]> = {
  ...
  "38-multimodal": [
    {
      question: "What does a multimodal embedding model let you do that a text-only model can't?",
      options: [
        "Embed text faster",
        "Put text, images, and video directly into the same vector space",
        "Skip chunking entirely",
        "Avoid needing a vector database",
      ],
      correctIndex: 1,
      explanation: "A multimodal embedding model maps text, images, and video into one shared vector space, so a query in one modality can retrieve a match in another — no separate extraction step needed where the cloud offers this.",
    },
    {
      question: "Why is \"extraction, then embed\" still the default for video across most clouds?",
      options: [
        "Native multimodal video embedding is the industry exception, not a shortcut you're missing",
        "Extraction is always cheaper than native embedding",
        "Vector databases require pre-extracted content",
        "Video can't be embedded at all",
      ],
      correctIndex: 0,
      explanation: "Only GCP's Gemini embeds raw video directly today — AWS, Azure, and OSS all pull a transcript and keyframes first, then embed each piece separately, because native multimodal video embedding isn't yet the norm.",
    },
    {
      question: "What's the risk of mixing text-only and multimodal embeddings in one index with no modality tag?",
      options: [
        "Nothing — nearest-neighbor search handles mixed embeddings automatically",
        "Storage costs double",
        "Nearest-neighbor search can't tell the embeddings apart, so retrieval quality degrades silently",
        "The index becomes read-only",
      ],
      correctIndex: 2,
      explanation: "Vectors from different embedding spaces aren't directly comparable — without a modality tag to filter or route by, similarity search mixes incompatible vectors and returns poor matches with no visible error.",
    },
    {
      question: "Why doesn't better OCR fix retrieval on a scanned diagram?",
      options: [
        "OCR is always accurate enough",
        "A diagram needs an image embedding to capture layout and visual meaning — OCR only extracts text, missing what the diagram shows",
        "Scanned diagrams can't be indexed at all",
        "OCR is more expensive than image embedding",
      ],
      correctIndex: 1,
      explanation: "Layout-aware PDF parsing (Module 15) helps with text-heavy scans, but a diagram's meaning lives in its visual structure — a damaged roof, a wiring path — which only an image embedding captures.",
    },
    {
      question: "What's the real added cost of an extract-then-embed pipeline (vs. native multimodal embedding)?",
      options: [
        "Nothing — extraction is free",
        "A new failure surface: a bad transcript or a mistimed keyframe degrades retrieval quality invisibly, on top of the extra API cost",
        "It only adds latency, never a quality risk",
        "It doubles storage costs",
      ],
      correctIndex: 1,
      explanation: "Beyond the extra $0.05–$0.06 per clip on AWS/Azure, the two-stage pipeline adds a failure surface — a bad transcript or mistimed keyframe silently degrades retrieval until someone asks the question that exposes it.",
    },
  ],
};
