...
export const flavor: Record<string, FlavorEntry> = {
  ...
  "85-agents": {
    ...
    snippet: `import boto3\nbedrock = boto3.client("bedrock-agent")\n\nbedrock.create_agent_action_group(\n    agentId=agent_id,\n    agentVersion="DRAFT",\n    actionGroupName="search_docs",\n    apiSchema={"s3": {"s3BucketName": "gdp-schemas", "s3ObjectKey": "search-docs.json"}},\n    actionGroupExecutor={"lambda": search_docs_lambda_arn},\n)\n# The agent now plans, calls search_docs, and observes the result before responding\n\n# --- Scoped credentials: STS assume-role, not a long-lived IAM user key ---\n# The action-group Lambda assumes a narrowly-scoped role for just this call —\n# a 15-minute session token, not a static access key, and the role's policy\n# grants read-only access to the one OpenSearch index it needs.\nsts = boto3.client("sts")\nscoped_session = sts.assume_role(\n    RoleArn="arn:aws:iam::123456789012:role/search-docs-tool-readonly",\n    RoleSessionName="agent-search-docs-call",\n    DurationSeconds=900,\n)\ncreds = scoped_session["Credentials"]\nopensearch_client = boto3.client(\n    "opensearchserverless",\n    aws_access_key_id=creds["AccessKeyId"],\n    aws_secret_access_key=creds["SecretAccessKey"],\n    aws_session_token=creds["SessionToken"],\n)\n# search_docs's actual retrieval call uses this scoped session, not the Lambda's own role`,
    labSteps: [
      ...
      "Assume the agent's action-group execution role via STS with a scoped, time-limited session instead of a long-lived IAM user key, and confirm a call outside that role's permissions is denied.",
    ],
