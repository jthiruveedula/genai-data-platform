...
export const flavor: Record<string, FlavorEntry> = {
  ...
  "85-agents": {
    ...
    snippet: `from vertexai.generative_models import GenerativeModel, Tool, FunctionDeclaration\n\nsearch_docs = FunctionDeclaration(\n    name="search_docs",\n    description="Search the indexed document store for relevant chunks",\n    parameters={"type": "object", "properties": {"query": {"type": "string"}}},\n)\nmodel = GenerativeModel("gemini-3-flash", tools=[Tool(function_declarations=[search_docs])])\nresponse = model.generate_content(user_message)\n# response.candidates[0].function_calls -> runtime executes search_docs, feeds result back\n\n# --- Scoped credentials: Workload Identity Federation, not a service-account key ---\n# The agent's runtime SA impersonates a narrowly-scoped SA for just this tool call —\n# no long-lived JSON key ever touches disk, and the impersonated SA's IAM binding\n# grants read-only access to the one Vector Search index it needs, nothing wider.\nfrom google.auth import impersonated_credentials, default\n\nsource_credentials, _ = default()\nscoped_credentials = impersonated_credentials.Credentials(\n    source_credentials=source_credentials,\n    target_principal="search-docs-tool@gdp-project.iam.gserviceaccount.com",\n    target_scopes=["https://www.googleapis.com/auth/cloud-platform.read-only"],\n    lifetime=300,  # 5-minute token — re-minted per call, never cached long-term\n)\n# search_docs's actual retrieval call uses scoped_credentials, not the agent's own identity`,
    labSteps: [
