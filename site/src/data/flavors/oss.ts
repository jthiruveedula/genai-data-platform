...
export const flavor: Record<string, FlavorEntry> = {
  ...
  "85-agents": {
    ...
    snippet: `import litellm\n\ntools = [{\n    "type": "function",\n    "function": {"name": "search_docs", "description": "Search the indexed document store", "parameters": {...}},\n}]\nresponse = litellm.completion(model="ollama/qwen3", messages=messages, tools=tools)\n# If the model emits a tool call, run it via an MCP server and feed the result back into messages\ntool_result = mcp_client.call_tool("search_docs", response.tool_calls[0].function.arguments)\n\n# --- MCP server side: OAuth 2.1 resource-server check on the tool itself ---\n# The agent never gets a long-lived key — Keycloak/Auth0 (the authorization\n# server) issues it a short-lived, scoped token; this server only validates.\nfrom fastapi import Depends, FastAPI, Header, HTTPException\nimport jwt\n\napp = FastAPI()\nJWKS_CLIENT = jwt.PyJWKClient("https://keycloak.local/realms/gdp/protocol/openid-connect/certs")\n\ndef verify_scoped_token(authorization: str = Header(...), required_scope: str = "mcp:search_docs:read"):\n    token = authorization.removeprefix("Bearer ").strip()\n    signing_key = JWKS_CLIENT.get_signing_key_from_jwt(token)\n    claims = jwt.decode(token, signing_key.key, algorithms=["RS256"], audience="gdp-mcp-server")\n    if required_scope not in claims.get("scope", "").split():\n        raise HTTPException(status_code=403, detail=f"token missing required scope: {required_scope}")\n    return claims\n\n@app.post("/tools/search_docs")\ndef search_docs(query: str, claims: dict = Depends(verify_scoped_token)):\n    # claims is only reached if the bearer token is valid, unexpired, and scoped —\n    # an expired or under-scoped token 403s before this line ever runs.\n    return run_module25_retrieval(query)`,
    labSteps: [
