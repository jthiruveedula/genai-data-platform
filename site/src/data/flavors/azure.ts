...
export const flavor: Record<string, FlavorEntry> = {
  ...
  "85-agents": {
    ...
    snippet: `from azure.ai.projects import AIProjectClient\n\nagent = project_client.agents.create_agent(\n    model="gpt-5.6-luna",\n    name="gdp-rag-agent",\n    instructions="Answer using the search_docs tool before responding.",\n    tools=[{"type": "function", "function": search_docs_schema}],\n)\n# Foundry Agent Service runs the plan -> call search_docs -> observe -> respond loop\n\n# --- Scoped credentials: Managed Identity, not a shared app-registration secret ---\n# The agent's compute (Container App / Function) is granted a User-Assigned\n# Managed Identity scoped to Search Index Data Reader on just the one index\n# search_docs needs -- no client secret ever touches config or code.\nfrom azure.identity import ManagedIdentityCredential\nfrom azure.search.documents import SearchClient\n\ncredential = ManagedIdentityCredential(client_id="search-docs-tool-identity-client-id")\nsearch_client = SearchClient(\n    endpoint="https://gdp-search.search.windows.net",\n    index_name="gdp-chunks",\n    credential=credential,  # token minted per call, scoped by the identity's RBAC role\n)\n# search_docs's actual retrieval call uses search_client, never a static API key`,
    labSteps: [
