/* ============================================================================
   VENDORED — scroll-world scrub engine (MIT, © 2026 cyw), from the
   `scroll-world` Claude Code plugin, plus an ESM export at the bottom of the
   file. Kept byte-identical otherwise so upstream fixes can be re-applied by
   re-copying: local behaviour is themed with CSS variables and configured from
   `src/pages/world.astro`, never by editing this file.

   What the skill's own pipeline generated with a paid image/video API, this
   site renders locally and deterministically instead — see
   `tools/scroll-world/README.md`. The engine is indifferent to where the clips
   came from; it only requires that consecutive clips share a frame.
   ----------------------------------------------------------------------------
